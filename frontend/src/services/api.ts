...
async function tryRequest<T>(method: string, path: string, body?: unknown, mockFallback?: () => T): Promise<T> {
  try {
...
export const authApi = {
  ...
  register: (data: { name: string; email: string; password: string; role?: 'staff' | 'manager' }) =>
    request<AuthResponse>('POST', '/auth/register', data),
};

export const categoriesApi = {
  list: () => tryRequest<typeof mockCategories>('GET', '/categories', undefined, () => _mockCategories),
  get: (id: number) => tryRequest('GET', `/categories/${id}`, undefined, () => _mockCategories.find(c => c.id === id)),
  ...
  update: (id: number, data: { name: string; description: string }) =>
    tryRequest('PUT', `/categories/${id}`, data, () => {
      _mockCategories = _mockCategories.map(c => (c.id === id ? { ...c, ...data } : c));
...
export const ingredientsApi = {
  list: () => tryRequest<typeof mockIngredients>('GET', '/ingredients', undefined, () => _mockIngredients),
  get: (id: number) => tryRequest('GET', `/ingredients/${id}`, undefined, () => _mockIngredients.find(i => i.id === id)),
  ...
  update: (id: number, data: { name: string; unit: string; stock_quantity: number }) =>
    tryRequest('PUT', `/ingredients/${id}`, data, () => {
      _mockIngredients = _mockIngredients.map(i => (i.id === id ? { ...i, ...data } : i));
...
export const menuItemsApi = {
  list: () => tryRequest<typeof mockMenuItems>('GET', '/menu-items', undefined, () => _mockMenuItems),
  get: (id: number) => tryRequest('GET', `/menu-items/${id}`, undefined, () => _mockMenuItems.find(m => m.id === id)),
  create: (data: {
    name: string;
    description: string;
    price: number;
  ...
  update: (id: number, data: Partial<{ name: string; description: string; price: number; category_id: number; image_url: string; status: string; ingredients: number[] }>) =>
    tryRequest('PUT', `/menu-items/${id}`, data, () => {
      _mockMenuItems = _mockMenuItems.map(m => (m.id === id ? { ...m, ...data } : m));
...
export const ordersApi = {
  list: (params?: { status?: string; from?: string; to?: string }) => {
    ...
    return tryRequest<typeof mockOrders>('GET', `/orders${query}`, undefined, () => {
      let orders = [..._mockOrders];
      if (params?.status) orders = orders.filter(o => o.status === params.status);
      return orders;
    });
  },
  get: (id: number) => tryRequest('GET', `/orders/${id}`, undefined, () => _mockOrders.find(o => o.id === id)),
  create: (data: { table_id: number; items: { menu_item_id: number; quantity: number; price: number }[] }) =>
    tryRequest('POST', '/orders', data, () => {
      ...
      const menuItemMap = Object.fromEntries(_mockMenuItems.map(m => [m.id, m.name]));
      const order = {
        id: _nextId++,
        table_id: data.table_id,
        table_number: table?.number || 0,
        items: data.items.map(i => ({ ...i, name: menuItemMap[i.menu_item_id] || '' })),
      ...
      if (table) {
        _mockTables = _mockTables.map(t => (t.id === data.table_id ? { ...t, status: 'occupied' } : t));
      }
      return order;
    }),
...
export const tablesApi = {
  list: () => tryRequest<typeof mockTables>('GET', '/tables', undefined, () => _mockTables),
  get: (id: number) => tryRequest('GET', `/tables/${id}`, undefined, () => _mockTables.find(t => t.id === id)),
  ...
  update: (id: number, data: { number: number; capacity: number; status: string }) =>
    tryRequest('PUT', `/tables/${id}`, data, () => {
      _mockTables = _mockTables.map(t => (t.id === id ? { ...t, ...data } : t));
...
export const reportsApi = {
  summary: () => tryRequest('GET', '/reports/summary', undefined, () => mockReports.summary),
  byCategory: () => tryRequest('GET', '/reports/by-category', undefined, () => mockReports.by_category),
  trend: () => tryRequest('GET', '/reports/trend', undefined, () => mockReports.trend),
  inventory: () => tryRequest('GET', '/reports/inventory', undefined, () => mockReports.inventory),
};
