...
export default function TablesPage() {
  ...
  const [form, setForm] = useState<FormState>({ number: '', capacity: '4', status: 'available' });
  ...
  const validate = () => {
    ...
    if (isNaN(num) || num < 1) e.number = 'Enter a valid table number';
    else if (!editing && tables.find(t => t.number === num)) e.number = 'Table number already exists';
    if (isNaN(cap) || cap < 1) e.capacity = 'Enter a valid capacity';
    return e;
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (editing) {
        await tablesApi.update(editing.id, { number: parseInt(form.number), capacity: parseInt(form.capacity), status: form.status });
        showToast('Table updated', 'success');
      } else {
        await tablesApi.create({ number: parseInt(form.number), capacity: parseInt(form.capacity) });
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="card">
          <EmptyState
            ...
            action={<button onClick={openAdd} className="btn-primary"><Plus size={14} />Add Table</button>}
          />
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            ...
            >
              <div className="text-center mb-3">
                <div className={`w-14 h-14 rounded-2xl mx-auto flex items-center justify-center mb-2 ${
                  table.status === 'occupied' ? 'bg-amber-100' : 'bg-terracotta/10'
                }`}>
                  <span className={`font-serif text-2xl font-bold ${
                    table.status === 'occupied' ? 'text-amber-700' : 'text-terracotta'
                  }`}>{table.number}</span>
                </div>
              ...
              <div className="flex gap-1 justify-center">
                ...
                <button onClick={() => openEdit(table)} className="flex-1 btn-ghost py-1 text-xs justify-center">
                  <Pencil size={13} />
                </button>
                <button onClick={() => setDeleteTarget(table)} className="flex-1 btn-danger py-1 text-xs justify-center">
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Table' : 'Add Table'} size="sm">
      ...
      <Modal open={!!qrModal} onClose={() => setQrModal(null)} title={`QR Code — Table ${qrModal?.number}`} size="sm">
        {qrModal && (
          <div className="flex flex-col items-center gap-4 py-2">
            <div className="p-4 bg-white rounded-xl border-2 border-cream-200 shadow-inner">
              <img src={qrModal.qr_code} alt={`QR Table ${qrModal.number}`} className="w-40 h-40" />
            </div>
            <p className="text-sm text-espresso-500">Scan to view the menu for Table {qrModal.number}</p>
            <a href={qrModal.qr_code} download={`table-${qrModal.number}-qr.png`} className="btn-secondary text-xs">
              Download QR Code
            </a>
          </div>
        )}
