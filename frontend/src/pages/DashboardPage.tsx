  ...
  const kpis = summary
    ? [
        ...
        },
        {
          ...
          icon: Star,
          sub: `${summary.top_items[0]?.count || 0} đơn`,
          color: "text-sky-600",
          bg: "bg-sky-50",
        },
      ]
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-espresso-400 mt-0.5">
          Bức tranh hoạt động quán cà phê
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-5">
          <h3 className="font-serif text-lg font-semibold text-espresso mb-4">
          ...
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="h-10 bg-cream-100 rounded-lg animate-skeleton"
                />
              ))}
            </div>
          ) : (
                ...
                <div key={item.name} className="flex items-center gap-3">
                  ...
                  <div className="w-20 h-1.5 bg-cream-200 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(item.count / (summary?.top_items[0]?.count || 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          {loading ? (
            <div className="space-y-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-9 bg-cream-100 rounded-lg animate-skeleton"
                />
              ))}
            </div>
          ) : (
